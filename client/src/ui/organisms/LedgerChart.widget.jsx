...
export const LedgerChartWidget = () => {
    ...
    const { isLoading, error, data: dataFetch} = useQuery(SUMMARY_QUERY, async () => {
    ...
    const validData = () => dataFetch && Object.values(dataFetch).every(el => {
      if (typeof el === "object") {
        return !!el.length
      }
        ...
        datasets: [
          {
            ...
            backgroundColor: dataFetch?.spending.map((item,index) => item.categoryColor ),
            borderWidth: 1,
          },
        ],
      };
    ...
    return (
        ...
        <Card>
          <div style={{display:'flex', alignItems: 'center', justifyContent:'space-between'}}>
            ...
            <b><Money inCents={dataFetch?.balance}/></b> 
          </div>
          ...
          {
          dataFetch?.spending.map((item, index) => <CategoryCell size={'12px'} key={index} color={item.categoryColor} name={item.categoryName} /> )
          }
        </Card>
    )
}
